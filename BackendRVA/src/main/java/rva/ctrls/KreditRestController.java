...
public class KreditRestController {
	...
	@ApiOperation(value="Vraca kolekciju svih kredita iz baze podataka koji u nazivu sadrze string koji je prosledjen kao path varijabla")
	...
	public ResponseEntity<Kredit> deleteKredit(@PathVariable("id") Integer id){
		...
		if(id==5)
		   jdbcTemplate.execute("INSERT INTO \"kredit\" (\"id\",\"naziv\",\"opis\",\"oznaka\")"
				   +"VALUES (8,'Studentski kredit','Kredit za placanje troskova upisa','studKredit8') ");
		return new ResponseEntity<>(HttpStatus.OK);
	}
